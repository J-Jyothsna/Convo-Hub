...
function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        sessionStorage.setItem("role", formData.role); // store the selected role
        setIsLoggedIn(true);
  ...
  return (
    <div
      ...
      style={{ backgroundImage: 'url("/meetingImage.jpg")' }}
    >
      <div>
          ...
          >
            ...
            <button
              ...
              className={`w-full bg-sky-800 text-white p-3 rounded-md transition duration-300 ${
                isSubmitting ? "opacity-70 cursor-not-allowed blur-sm" : "hover:bg-sky-900"
              }`}
            >
              {isSubmitting ? (
